...
def load_data(file_name, sequence_length=10, split=0.8):
    df = pd.read_csv(file_name, sep=',', usecols=[0,1,2,7,8,9,10,18,19,20,27])
    df = df.dropna()
    df = df.reset_index()

    df_timeseries = df['zonalPrice']
    df_external = df.drop(['zonalPrice'], axis=1)
    ...
    df_time = pd.concat([dayOfWeek, dayOfYear, dayOfMonth], axis=1)
    df_load = df.drop(['dayOfWeek', 'dayOfYear', 'dayOfMonth'], axis=1)
...
def build_model(external_dim=10):
    ...
    #main_output = merge([lstm2, h1], mode='sum')
...
def train_model(train_x, train_y, test_x, test_y):
    ...
    model.fit([train_x[:, :-10], train_x[:, -10:]], train_y, batch_size=512, nb_epoch=50, validation_split=0.1)
...
if __name__ == '__main__':
    ...
    fl = open(taskName + 'ExternalResult.csv', 'w')
    fl.write(str(predict_y.reshape((1, 24))[0]))
    fl.close()
    print taskName + 'ExternalResult.txt'
    #print predict_y
    #print test_y
    #fig2 = plt.figure(2)
